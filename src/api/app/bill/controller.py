...
#CREATE BILLS
def create_bill(body,incident_id,role_id):
    try: 
        if verify_admin(role_id):
            new_bill=Bill(amount=body["amount"],detail=body["detail"],provider_id=body["provider_id"])
...
#DELETE ONE BILL
def delete_bill(role_id,bill_id):
    try:
        if verify_admin(role_id):
